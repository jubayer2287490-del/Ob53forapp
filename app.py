    ...
    if action in ["start", "restart"]:
        if name in user_processes: 
            try:
                ...
                user_processes[name]['proc'].wait()
            except: pass
        try:
            with open("bot.txt", "w") as f: f.write(f"uid={uid}\npassword={pw}\n")
            proc = subprocess.Popen([sys.executable, '-u', 'main.py'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1, universal_newlines=True)
